...
export default function CarsForHire({ filterMake = '' }) {
  ...
  const { data: cars = [], isLoading, error } = useQuery({
    queryKey: QUERY_KEYS.carsHire,
    queryFn:  () => fetchCars('hire'),
  })
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteCar(id)
      queryClient.setQueryData(QUERY_KEYS.carsHire, (old) =>
  ...
  if (!filtered.length) return (
    <div style={{ textAlign: 'center', padding: '60px 0', fontFamily: "'Barlow', sans-serif", color: '#6080b0', fontSize: 14 }}>
      {filterMake ? `No "${filterMake}" cars for hire.` : 'No hire cars available right now.'}
    </div>
  )
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(290px, 1fr))', gap: 24 }}>
        {filtered.map((car, i) => (
          <div key={car.id} style={{
            ...
            <CarCard car={car} onShowMore={setSelectedCar} onEdit={handleEdit} onDelete={handleDelete} isHire={true} />
          </div>
        ))}
      </div>
      {selectedCar && <CarModal car={selectedCar} onClose={() => setSelectedCar(null)} isHire={true} onShowMore={setSelectedCar} />}
    </section>
  )
}