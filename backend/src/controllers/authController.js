...
async function login(req, res) {
  ...
  try {
    const result = await pool.query(
      'SELECT * FROM dealers WHERE email = $1', [email]
    )
...
// POST /api/auth/register  (protect this in production!)
async function register(req, res) {
  ...
  try {
    ...
    const result = await pool.query(
      'INSERT INTO dealers (name, email, password) VALUES ($1, $2, $3) RETURNING id, name, email',
      [name, email, hashed]
    )
    const dealer = result.rows[0]
